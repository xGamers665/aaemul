...
{
    public class EditorProtocolHandler : BaseProtocolHandler
    {
        private static Logger _log = LogManager.GetCurrentClassLogger();

        private ConcurrentDictionary<uint, Type> _packets;

        public EditorProtocolHandler()
        {
        ...
        public override void OnConnect(Session session)
        {
            ...
            try
            {
                var con = new EditorConnection(session);
                con.OnConnect();
                EditorConnectionTable.Instance.AddConnection(con);
            }
            catch (Exception e)
            {
                session.Close();
        ...
        public override void OnDisconnect(Session session)
        {
            try
            {
                var con = EditorConnectionTable.Instance.GetConnection(session.Id);
                if (con != null)
                    EditorConnectionTable.Instance.RemoveConnection(session.Id);
            }
            catch (Exception e)
            {
                session.Close();
        ...
        public override void OnReceive(Session session, byte[] buf, int bytes)
        {
            try
            {
                var connection = EditorConnectionTable.Instance.GetConnection(session.Id);
                if (connection == null)
                    return;
        ...
        public void OnReceive(EditorConnection connection, byte[] buf, int bytes)
        {
            try
            {
                ...
                while (stream != null && stream.Count > 0)
                {
                    ...
                    if (packetLen <= stream.Count)
                    {
                        ...
                        {
                            var packet = (EditorPacket) Activator.CreateInstance(classType);
        ...
        private void HandleUnknownPacket(EditorConnection connection, uint type, PacketStream stream)
        {
