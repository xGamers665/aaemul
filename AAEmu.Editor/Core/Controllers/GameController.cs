...
namespace AAEmu.Editor.Core.Controllers
{
    public class GameController : Singleton<GameController>
    {
        ...
        public void RequestWorldList(EditorConnection connection)
        {
        ...
        public void RequestEnterWorld(EditorConnection connection, byte gsId)
        {
            if (!_gameServers.ContainsKey(gsId))
                return;
        ...
        public void EnterWorld(EditorConnection connection, byte gsId, byte result)
        {
            if (result == 0)
            {
                if (_gameServers.ContainsKey(gsId))
